...
def preprocess_evidence(evidence: str, schema_items: list[dict[str, str]]) -> str:
    ...
    return evidence
def prepare_text2sql_prefix_sequence(data: dict[str, str]) -> str:
    prefix_seq = data["schema_sequence"] + "\n" + data["content_sequence"] + "\n" + data["text"] + "\n"
    
    return prefix_seq

def prepare_inputs_and_labels(prefix_seq: str, target_seq: str, tokenizer, max_tokens: int): # type: ignore
    prefix_ids = [tokenizer.bos_token_id] + tokenizer(prefix_seq , truncation = False)["input_ids"] # type: ignore
    target_ids = tokenizer(target_seq, truncation = False)["input_ids"] + [tokenizer.eos_token_id] # type: ignore

    seq_length = len(prefix_ids) + len(target_ids) # type: ignore
    if seq_length <= max_tokens: # pad inputs with pad_token_id
        pad_length = max_tokens - seq_length
        input_ids = prefix_ids + target_ids + [tokenizer.pad_token_id] * pad_length # type: ignore
        ...
        labels = [-100] * len(prefix_ids) + target_ids + [-100] * pad_length # type: ignore
    else: # no padding
        print("the current input sequence exceeds the max_tokens, we will truncate it.")
        input_ids = prefix_ids + target_ids # type: ignore
        # pre-truncate input ids
        input_ids = [tokenizer.bos_token_id] + input_ids[-(max_tokens-1):] # type: ignore
        attention_mask = [1] * max_tokens
        # only target_ids produces gradients
        labels = [-100] * len(prefix_ids) + target_ids # type: ignore
        # pre-truncate labels
        labels = labels[-max_tokens:] # type: ignore
    
    return {
        "input_ids": torch.tensor(input_ids, dtype = torch.int64), # type: ignore
        "attention_mask": torch.tensor(attention_mask, dtype = torch.int64), # type: ignore
        "labels": torch.tensor(labels, dtype = torch.int64) # type: ignore
    }

def prepare_inputs(prefix_seq: str, tokenizer, max_prefix_length: int): # type: ignore
    input_ids = [tokenizer.bos_token_id] + tokenizer(prefix_seq , truncation = False)["input_ids"] # type: ignore

    if len(input_ids) > max_prefix_length: # type: ignore
    ...
    attention_mask = [1] * len(input_ids) # type: ignore
...
def create_and_run_conv_dataset(
    ...
    '''
    Load spider-style dataset and run the text-to-SQL model to predict SQLs for each question.
    '''

    dataset: "list[dict[str,str|dict[str,str]|list[dict[str,str|dict[str,str]]]]]" = json.load(open(dataset_path))
    ...
    db_ids: "set[str]" = set([str(data["database_id"]) for data in dataset])
    db_id2searcher: "dict[str, LuceneSearcher]" = dict()
    for db_id in db_ids:
        db_id2searcher[db_id] = LuceneSearcher(os.path.join(db_content_index_path, db_id)) # type: ignore

    db_id2schema: "dict[str | dict[str, str] | list[dict[str, str | dict[str, str]]], str]" = dict()
    count = 0
    predicted_sqls = []
    # print(source)
    # exit()
    if "cosql" in source or "sparc" in source: # preprocess cosql to make each interaction split into individual samples
        for data in tqdm(dataset):
            ...
            db_id = str(data["database_id"])
            history: "list[str]" = []
            for q in data['interaction']:
                assert isinstance(q, dict), "type issue"
                sample: "dict[str,str]" = {}
                sample["db_id"] = db_id
                sample['question'] = ""
                if history:
                    sample['question'] += '\n'.join(history)+"\n"
                    sample["question"] += str(q['utterance'])
                else:
                    sample["question"] += str(q['utterance'])
                sample["query"] = str(q['query'])

                # for i in range(3000):
                #     sample['question'] += "\nfiller text"

                history.append(str(q['utterance']))
                # history.append(q['query'])
                # rewrite question
                from GQR import GQR
                print()
                print('Original Question: \n',sample['question'].replace("\n"," "))
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(opt.llm_path) #type: ignore
    model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.float16) #type: ignore
